...
this.c=this.can.getContext("2d");this.c.beginPath();}
  ...
  fd(len,str2a) {this.c.strokeStyle=str2a;this.xf=this.xs+len*Math.cos(Math.PI*this.angle/180)
  this.yf=this.ys+len*Math.sin(Math.PI*this.angle/180)
  this.c.moveTo(this.xs,this.ys);
  this.c.lineTo(this.xf,this.yf);
  this.c.stroke();
...
poly(len,n){
	this.fd(len/(2*Math.tan((Math.PI/180)*(180/n))));
	this.rt(90);this.bk(len/2);
for(let j=0;j<n;j++){
	this.fd(len);this.rt(360/n);
}	
this.fd(len/2);this.lt(90);
this.bk(len/(2*Math.tan((Math.PI/180)*(180/n))));
}
}
